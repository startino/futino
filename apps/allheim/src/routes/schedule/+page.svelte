...
<section
	class="py-20 px-4 border-b shadow-2xl sm:px-6 md:px-8  border-primary/40">
	<div class="grid justify-items-center space-y-12 mx-auto">
		...
		<input
			id="name"
			class=" bg-primary p-2 text-center  placeholder-primary-on/60  text-primary-on headline-large"
...
<section class="py-5 px-4 shadow-2xl sm:px-6 md:px-8">
	<div class="grid grid-cols-2 justify-items-center">
		<button
			on:click={register}
			id="register"
			class="p-5 shadow-lg rounded-lg text-center  bg-secondary  text-secondary-on display-small">
			Register
		</button>
			...
			{#each weeks as week, index (index)}
				<div
					...
					{#each week as { day, month, date }}
						<label
							class="h-full w-full p-3 shadow-lg rounded-lg text-center  bg-secondary  text-secondary-on">
