<script lang="ts">
	...
	function startSession() {
		...
		const url = `${PUBLIC_API_BASE_URL}/maeve?id=${data.maeveId}&profile_id=${data.profileId}`;
	...
	function replySession(message: string) {
		...
		awaitingReply = false;
		const url = `${PUBLIC_API_BASE_URL}/maeve?id=${data.maeveId}&profile_id=${data.profileId}&session_id=${data.session.id}&reply=${message}`;
