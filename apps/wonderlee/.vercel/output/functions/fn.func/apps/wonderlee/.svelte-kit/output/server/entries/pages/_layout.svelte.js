import { c as create_ssr_component, e as escape, v as validate_component, b as each, d as add_attribute, f as add_styles, g as compute_slots } from "../../chunks/ssr.js";
...
const Logo = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return ` <img src="/favicon.png" class="${escape($$props.class, true) + " " + escape(height, true)}" alt="Logo">`;
});
...
const Shell = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div id="appShell"${add_attribute("class", classesBase, 0)} data-testid="app-shell"> ${$$slots.header ? `<header id="shell-header" class="${[
    "flex-none " + escape(classesHeader, true) + " " + escape("opacity-0", true) + " transition-opacity",
    "invisible"
  ].join(" ").trim()}">${slots.header ? slots.header({}) : ``}</header>` : ``}  <div class="${[
    "flex-auto " + escape("opacity-0", true) + " transition-opacity duration-300 " + escape(cContentArea, true),
    "invisible"
  ].join(" ").trim()}"> ${$$slots.sidebarLeft ? `<aside id="sidebar-left"${add_attribute("class", classesSidebarLeft, 0)}>${slots.sidebarLeft ? slots.sidebarLeft({}) : ``}</aside>` : ``}  <div id="page" class="${escape(regionPage, true) + " " + escape(cPage, true)}"${add_styles({ "scrollbar-gutter": scrollbarGutter })}> ${$$slots.pageHeader ? `<header id="page-header" class="${"flex-none " + escape(classesPageHeader, true)}">${slots.pageHeader ? slots.pageHeader({}) : `(slot:header)`}</header>` : ``}  <main id="page-content" class="${"flex-auto " + escape(classesPageContent, true)}">${slots.default ? slots.default({}) : ``}</main>  ${$$slots.pageFooter ? `<footer id="page-footer" class="${"flex-none " + escape(classesPageFooter, true)}">${slots.pageFooter ? slots.pageFooter({}) : `(slot:footer)`}</footer>` : ``}</div>  ${$$slots.sidebarRight ? `<aside id="sidebar-right"${add_attribute("class", classesSidebarRight, 0)}>${slots.sidebarRight ? slots.sidebarRight({}) : ``}</aside>` : ``}</div>  ${$$slots.footer ? `<footer id="shell-footer" class="${"flex-none " + escape(classesFooter, true)}">${slots.footer ? slots.footer({}) : ``}</footer>` : ``}</div>`;
});
