<script lang="ts">
	...
	let miniHeaderClass = 'fixed py-3 border-b bg-black/60 backdrop-blur';
...
{#if menuOpen}
	...
	<div transition:fade class="fixed inset-0 z-50">
		...
		<div
			...
			out:slide
			class="absolute bottom-0 left-0 right-0 top-0 z-50 mx-auto my-auto flex h-fit w-full max-w-xs flex-col items-center gap-4 rounded-lg bg-black p-6 pb-8 shadow-lg md:max-w-xl lg:max-w-2xl"
		>
			<!-- Close Menu Button -->
			<button
				class="hover:stroke-tertiary my-4 flex h-7 w-7 items-center justify-center stroke-white transition-all duration-200 hover:scale-150"
				on:click={toggleMenu}
			>
			...
			<nav class="headline-medium flex flex-col gap-6 text-center">
				{#each elements as element}
					<a
						class="hover:text-tertiary text-white transition-all duration-200 hover:scale-105"
						href={element.href}
						on:click={toggleMenu}
					>
						{element.name}
...
<header
	id="header"
	class="{$$props.class} {activeheaderClass} duration-400 top-0 z-40 w-full border-white/10 text-white transition-all"
>
	<div class="md:px-18 relative mx-auto flex items-center px-6 lg:max-w-5xl xl:max-w-6xl">
		...
		<div class="title-large ml-auto flex items-center gap-8">
			...
			<div class="flex gap-6 md:border-white/50">
				<button class="flex stroke-white md:hidden" on:click={toggleMenu}>
