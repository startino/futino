...
<div
  class="{$$props.class} justify-center flex bg-surface border rounded-lg border-secondary/20"
>
  <div class="relative flex flex-row gap-1 items-center text-center mx-1 my-2">
    ...
    {#each promotions as { label, discount, index }, i}
      <!--{activeTabValue == promotion.index
					...
					: -->
      <button
        class="py-2 rounded-lg px-2 sm:px-5 md:px-8 {activeTabValue == i
          ? ''
      ...
      >
        <div class="flex flex-row gap-2">
          <p class=" text-white tracking-wide ">{label}</p>
