...
export const load = async ({ locals: { supabase, organization } }) => {
	...
	try {
		const { data, error: contractError } = await getReportContracts({
			supabase,
		...
		const { data: snapshotList, error: listError } = await supabase.storage
		...
		console.error('Error while getting report contracts', { e });
