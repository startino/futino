...
<div class={classesBase}>
	<label
		for="location"
		class="text-surface-on block text-sm font-medium leading-6 {uppercase ? 'uppercase' : ''}"
		>{label}</label
	>
	<select
		id="location"
		name="location"
		bind:value={selected}
		class="bg-surface text-surface-on ring-surface-on focus:ring-surface-on mt-2 block w-full rounded-md border-0 py-1.5 pl-3 pr-10 ring-inset focus:ring-2 sm:leading-6"
	>
