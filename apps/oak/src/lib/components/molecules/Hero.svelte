...
<Section
	class="z-0 h-screen w-full {typography}"
	style="{$$props.style} ; clip-path: inset(0 0 0 0);"
>
	<div class="{bgCover} absolute z-0 h-full w-full" />
	<div
		class="{parallax
			? 'fixed'
			: 'absolute '}  not-prose -z-10 h-full w-full transition-transform duration-700"
	...
	>
		{#if bgVideo}
			<video
				...
				autoplay
				loop
				class="h-screen max-h-screen w-full object-cover object-center"
			>
				<source
					type="video/webm"
					title="Hero Video"
				...
				<source type="video/webm" media="(max-width: 799px)" src={bgVideo['md']} />
		...
		{#if bgImg}
			<img
				src={bgImg}
				alt=""
				class="h-screen max-h-screen w-full object-cover object-center mix-blend-darken"
			/>
		{/if}
		<slot />
	</div>
	<div
		class="z-10 flex w-full flex-col items-center {justifyClass()} mx-2 my-auto gap-12 transition-transform duration-700 sm:mx-4 md:mx-6 {parallax
			? 'fixed'
			: ''}"
		style={parallax && scrollY < 1000
			? 'transform:  scale(' +
