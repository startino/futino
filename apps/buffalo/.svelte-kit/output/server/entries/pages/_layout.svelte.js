...
function _layout($$payload, $$props) {
  ...
  const footerLinks = {
    Explore: { Home: "/", Classes: "/#classes" },
    Support: {
      About: "/about",
      "Find Us": "/contact#find-us",
      Contact: "/contact"
    },
    Legal: { "Privacy Policy": "/", "T&C and Waiver": "/" }
  };
  const contactItems = [
    {
      icon: "phone",
      ...
      "footer": ($$payload2, $$slotProps) => {
        {
          ...
          FatFooter($$payload2, {
            pages: footerLinks,
            $$slots: {
              "left": ($$payload3, $$slotProps2) => {
                {
                  const anchor_5 = create_anchor($$payload3);
                  const each_array = ensure_array_like(contactItems);
                  $$payload3.out += `<div class="flex flex-col gap-4">${anchor_5}`;
                  for (let $$index = 0; $$index < each_array.length; $$index++) {
                    const $$item = each_array[$$index];
                    const { icon, label, href } = $$item;
                    const anchor_6 = create_anchor($$payload3);
                    const anchor_7 = create_anchor($$payload3);
                    $$payload3.out += `${anchor_6}<a class="flex flex-row gap-4 place-items-center" style="margin: 0px"${attr("href", href, false)}>${anchor_7}`;
                    Icon($$payload3, { width: "24", height: "24", icon });
                    $$payload3.out += `${anchor_7} <h6 style="margin:0">${escape_text(label)}</h6></a>${anchor_6}`;
                  }
                  $$payload3.out += `${anchor_5}</div>`;
                }
              }
            }
          });
