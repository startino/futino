	...
	{#if !textArea}
		<label
			...
			class=" {value
				...
				: 'font-normal left-3 top-1/2'} block group-hover:font-bold text-outline-dark group-hover:text-primary-dark absolute -translate-y-1/2 duration-[400ms] ease-in-out group-hover:left-0 group-hover:-top-1/2 group-hover:-translate-y-2 transition-all"
			>{label}</label>
		...
		<input
			...
			bind:value
			class="w-full h-fit px-3 py-3 leading-tight text-secondary-dark bg-surface-dark border border-primary-dark rounded focus:shadow-glow focus:shadow-tertiary-dark/20 focus:outline-none appearance-none" />
	{:else}
		<label
			for={label.toLowerCase()}
			class="{value
				? 'font-bold text-primary-dark -top-2 -translate-y-full left-0'
				: 'font-normal left-3 top-2.5'} block group-hover:font-bold text-outline-dark group-hover:text-primary-dark absolute group-hover:-top-2 duration-[400ms] ease-in-out group-hover:-translate-y-full group-hover:left-0 transition-all"
			>{label}</label>
