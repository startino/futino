<script lang="ts">
	...
	let largeHeaderClass =
		'fixed text-primary py-8 md:bg-surface/60 md:shadow-2xl md:backdrop-blur md:drop-shadow-2xl';
	let activeheaderClass = largeHeaderClass;
	let miniHeaderClass =
		'fixed py-4 border-b text-primary bg-surface/60 bg-surface/60 backdrop-blur';
...
{#if menuOpen}
	...
	<div transition:fade class="fixed inset-0 z-50">
		...
		<div
			...
			out:slide
			class="absolute z-50 p-6 w-full mx-auto pb-8 my-auto top-0 bottom-0 flex flex-col items-center gap-4 left-0 right-0 h-fit max-w-xs md:max-w-xl lg:max-w-2xl bg-surface rounded-lg shadow-lg">
			<!-- Close Menu Button -->
			<button
				class="w-7 h-7 my-4 flex items-center hover:scale-150 transition-all duration-200 justify-center stroke-primary hover:stroke-tertiary"
				on:click={toggleMenu}>
...
<header
	...
	class="{$$props.class} {activeheaderClass} {menuOpen
		? 'opacity-0'
		: 'opacity-100'}  border-secondary/10 top-0 z-40 w-full transition-all duration-200">
	<div
		...
		<div class="flex items-center ml-auto gap-6">
			<nav class="hidden md:flex gap-6 pb-0.5">
				{#each elements as element, i}
					<a class=" hover:text-tertiary transition-all duration-300" href={element.href}>
						{element.name}
					</a>
				{/each}
			</nav>
			<div class="flex md:border-primary/50">
				<button
					class="flex md:hidden stroke-primary hover:stroke-tertiary transition-all"
					on:click={toggleMenu}>
